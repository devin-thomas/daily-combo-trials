...
- static/art/ contains source-derived roster artwork. Steam games use their app ID as the asset directory; non-Steam games use their stable game slug. The supplied 320px `previews/` thumbnails are not used; the checked-in WebP deliveries are sized to preserve detail while staying within Vercel Hobby's 100 MB upload limit.
...
- Uppercut Labs was explicitly selected in Safari, account
  `f81d8a09a3b225b740c17f5a39f6cf80`. The complete four-site inventory had no
  entry for this hostname, so non-proxied Web Analytics site
  `868e6f2af47b422485bf71b54c4a6dac` was created for exactly
  `daily-combo-trials.vercel.app`. Its generated beacon token was obtained,
  and the shared template matches the supplied `type="module"` snippet.
  DNS and hosting remain unchanged.
...
- Both providers' pageview collection is **verified**, with separate Safari network
  and dashboard evidence. The activation deployment is
  `dpl_9PpzoRwmBSZqgUougPP1pP2jL7we`, commit `09d17d0`. First observed acceptance:
  Vercel `/_vercel/insights/view` HTTP 200 at **2026-09-05 16:47:28 UTC**;
  Cloudflare `/cdn-cgi/rum` HTTP 204 at **16:47:31 UTC**. The small live smoke
  visited the homepage and History once each. Vercel displayed 2 pageviews and
  Cloudflare displayed 1 so far; these separate totals are not reconciled or added.
  Safari's homepage console was empty, and History navigation worked normally.
- Post-deployment HTTP checks confirmed one script per provider on home, game,
  character, and history pages, with real JavaScript responses. Setup remained
  denied (404; `/setup/` preserves its 307 to the denied route), with no analytics
